...
# --- GUARDIÁN DE SEGURIDAD (NUEVO) ---
# Esto verifica el estado de la sesión antes de dibujar el resto de la página
if not st.session_state.get("autenticado"):
    st.error("Acceso denegado. 🚫 Por favor, inicie sesión desde la página principal.")
    st.stop() # Detiene la ejecución de esta página si no está logueado

if st.session_state.get("perfil", {}).get("rol") != 'admin':
    st.error("Acceso denegado. Esta sección es solo para administradores. 🔒")
    st.stop() # Detiene si no es admin
